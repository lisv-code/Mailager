...
#include "fs_data.h" // "wx/fs_data.h" // since wxWidgets 3.3
...
// URL (location) syntax: data:[<mediatype>][;base64],<data>
...
wxFSFile* wxDataSchemeFSHandler::OpenFile(wxFileSystem& WXUNUSED(fs), const wxString& location)
{
	wxString mediatype;
	bool isBase64 = false;
	int dataPos = GetMetadata(location, mediatype, isBase64);

	if (dataPos < 0) return nullptr;

	if (mediatype.IsEmpty()) mediatype = "text/plain";
	...
	if (stream) {
		return new wxFSFile(stream, "", mediatype, ""
#if wxUSE_DATETIME
			, wxDateTime::Now()
