	...
	TimeZoneOptions tz_options)
{
	...
	buf << std::put_time((nullptr == date_time) ? &tm1 : date_time,
		(tzoLocal & tz_options) ? DateTimeFmtStr_DefaultWithTz : DateTimeFmtStr_Default);
	if (buf.good() && !buf.str().empty()) {
		date_time_string = buf.str();
		if ((tzoNone != tz_options) && !(tzoLocal & tz_options)) {
			...
			if (tzoUtc & tz_options) tz_str = " +0000";
				...
				if (0 != tz_val) {
					char tz_buf[0xF];
					std::sprintf(tz_buf, "%04d", std::abs(tz_val));
					tz_str = (tz_val >= 0 ? " +" : " -");
					tz_str += tz_buf;
				}
			}
...
{
	std::istringstream buf(dt_str);
	buf.imbue(std::locale::classic());
	int fmt_idx = 0;
	do {
		buf >> std::get_time(&dt_result, DateTimeFmtStr[fmt_idx]);
		if (buf.good()) break;
		else {
			buf.clear();
			++fmt_idx;
